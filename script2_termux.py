...
Creates a 7-day guide (Israel Sunday 00:00 → next Sunday 00:00) and writes file2.xml
"""
...
import cloudscraper, ssl, urllib3, certifi
...
HOT_API     = "https://www.hot.net.il/HotCmsApiFront/api/ProgramsSchedual/GetProgramsSchedual"  # HOT’s typo

IL_TZ         = ZoneInfo("Asia/Jerusalem")
CHANNELS_FILE = "/data/data/com.termux/files/home/eagle/channels.xml"
OUT_XML       = "/data/data/com.termux/files/home/eagle/file2.xml"
...
def fmt_ts(d,_site):  # XMLTV timestamp UTC
...
def fetch_cellcom(sess,site_id,since,till):
    ...
    # login
    r=sess.post(CELL_LOGIN,json={"apiVersion":"5.4.0.28193",
                                 "partnerId":"3197",
    ...
    objs=resp.get("objects") or resp.get("result",{}).get("objects",[])  # fallback keeps it working
...
def fetch_partner(sess,site_id,since,till):
    ...
    cur,out=since,[]
    while cur<till:
        out.extend(_day(cur)); cur+=dt.timedelta(days=1)
    uniq,seen=[],set()
    for it in out:
        k=(it.get("start"),it.get("end"),it.get("name"))
        if k not in seen:
...
def _collect_hot(sess,day):
    ...
    rows=[it for it in rows if str(it.get("programStartTime","")).startswith(day_prefix)]
    by={}
    for it in rows:
...
def fetch_hot(sess,site_id,start,till):
    ...
    while cur<till:
        day=cur.date()
        if day not in _HOT_CACHE:
            _HOT_CACHE[day]=_collect_hot(sess,cur)
        items.extend(_HOT_CACHE[day].get(site_id.zfill(3),[]))
        cur+=dt.timedelta(days=1)
    return items

# ───────── main build ─────────
def build_epg():
    since,till=week_window(dt.datetime.now(IL_TZ))
    sess=new_session()
    root=ET.Element("tv",{"source-info-name":"FreeTV+Cellcom+Partner+Yes+HOT (Week)",
                          "generator-info-name":"proxyEPG"})

    # map xmltv id → list of variants
    entries={}
