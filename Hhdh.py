...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "log_menu":
        jobs = get_jobs()
        keyboard = [
            [InlineKeyboardButton(job, callback_data=f"log_select:{job}:10"),
             InlineKeyboardButton(job, callback_data=f"log_select:{job}:20"),
             InlineKeyboardButton(job, callback_data=f"log_select:{job}:50")]
            for job in jobs
        ]
        keyboard.append([InlineKeyboardButton("⬅ Back", callback_data="start")])
        await query.edit_message_text("📄 Select job to view logs (10/20/50 lines):", reply_markup=InlineKeyboardMarkup(keyboard))
        return

    # -------- Cron Menu --------
    if data == "cron_menu":
        jobs = get_jobs()
        keyboard = [[InlineKeyboardButton(job, callback_data=f"cron_show:{job}")] for job in jobs]
        keyboard.append([InlineKeyboardButton("⬅ Back", callback_data="start")])
    ...
    # -------- Cron Show --------
    if data.startswith("cron_show:"):
        job = data.split(":")[1]
        cmd = get_job_command(job)
        if cmd:
            await query.edit_message_text(
                f"⏱ Current schedule for {job}:\n`{cmd}`\n\nTo change, type /cron manually for now."
            )
        else:
            await query.edit_message_text(f"❌ Job {job} not found")
        return
