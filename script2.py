#!/usr/bin/env python3
"""
EPG builder for FreeTV, Cellcom, Partner and Yes through an Israeli proxy.
"""
...
def to_dt(val):
    """Convert ISO (with or w/o Z) or unix-int to aware IL datetime."""
...
def fetch_yes(sess, site_id, since, till):
    chan = site_id.strip()
    date_str = since.strftime("%Y-%m-%d")      # UTC date requested
    url = f"{YES_CH_BASE}/{chan}?date={date_str}&ignorePastItems=true"
    r = sess.get(url, headers=YES_HEADERS, timeout=30); print(r.url); r.raise_for_status()
    data = r.json()
    # Response structure: {"id": "...", "items":[{...}, ...], ...}
...
def build_epg():
    ...
    programmes = []
    id_state: dict[tuple[str,str], bool] = {}   # (site, logical) -> hasData
