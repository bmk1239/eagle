...
Times are written with the IL offset (+0300 / +0200).  Output: file2.xml
"""
...
def to_dt_local(v):                       # -> datetime <tz=IL>
...
# ... (קוד הפונקציות fetch_cellcom / fetch_partner / fetch_yes נשאר כפי שהיה) ...
...
def build_epg():
    ...
    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        ...
        if not xmltv: dbg("skip","empty xmltv_id, ignored",flush=True); continue
    ...
    for xmltv, variants in entries.items():
        ok=None
        for site,raw,name,_ in variants:
            try:
            ...
            except Exception as e: dbg(site,"fetch error",xmltv,e,flush=True); items=[]
            dbg(site,f"{xmltv} → {len(items)} items",flush=True)
            if items: ok=(items,site,name); break
        if not ok: dbg("skip",f"{xmltv}: no data",flush=True); continue
        ...
        for it in items:
            try:
                if site=="freetv.tv":
                    s,e=to_dt_local(it["since"]),to_dt_local(it["till"]); title=it["title"]; desc=it.get("description") or it.get("summary")
                elif site=="cellcom.co.il":
                    s,e=to_dt_local(it["startDate"]),to_dt_local(it["endDate"]); title=it["name"]; desc=it.get("description")
                elif site=="partner.co.il":
                    s,e=to_dt_local(it["start"]),to_dt_local(it["end"]); title=it["name"]; desc=it.get("shortSynopsis")
                    ...
                    s,e=to_dt_local(it["starts"]),to_dt_local(it["ends"]); title=it["title"]; desc=it.get("description")

                # --- המרה ל-UTC לרישום בקובץ ---
                s_utc,e_utc=s.astimezone(UTC_TZ),e.astimezone(UTC_TZ)
                pr=ET.SubElement(root,"programme",
                                 start=s_utc.strftime("%Y%m%d%H%M%S +0000"),
                                 stop =e_utc.strftime("%Y%m%d%H%M%S +0000"),
                                 channel=xmltv)
