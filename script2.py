#!/usr/bin/env python3
"""
XMLTV guide builder for FreeTV + Cellcom via IL proxy.
...
# ─── helpers ───
def day_window(now): s=dt.datetime.combine(now.date(),dt.time.min,tzinfo=IL_TZ); return s,s+dt.timedelta(days=1)
def configure():
...
def fetch_cellcom(sess,site_id,since,till):
    ...
    # login
    r=sess.post(CELL_LOGIN,json={"apiVersion":"5.4.0.28193","partnerId":"3197","udid":"f442...d79"},
                headers=CELL_HEADERS,timeout=30); print(r.url); r.raise_for_status()
    ks=r.json().get("ks") or r.json().get("result",{}).get("ks")
    if not ks: raise RuntimeError("KS token missing")

    since_ts,till_ts=str(int(since.timestamp())),str(int(till.timestamp()))
    ...
    r=sess.post(CELL_LIST,json=payload,headers=CELL_HEADERS,timeout=30); print(r.url); r.raise_for_status()
...
def build():
    ...
    progs=[]
    for ch in ET.parse(CHANNELS).findall("channel"):
        site=ch.attrib.get("site","").lower(); sid=ch.attrib["site_id"]; xid=ch.attrib["xmltv_id"]
        name=(ch.text or xid).strip()
        ET.SubElement(ET.SubElement(root,"channel",id=xid),"display-name",lang="he").text=name
        try:
            if site=="freetv.tv": items=fetch_freetv(sess,sid,since,till)
            elif site=="cellcom.co.il": items=fetch_cellcom(sess,sid,since,till)
            else: dbg("unknown site",site); continue
            dbg(f"{xid} → {len(items)} items"); progs.extend((xid,i,site) for i in items)
        except Exception as e: dbg("fetch error",xid,e)
    ...
    ET.indent(root); ET.ElementTree(root).write(OUT,encoding="utf-8",xml_declaration=True)
    print("✅ wrote",OUT)
