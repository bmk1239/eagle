...
• Creates a one-day guide (IL-time today 00:00 → tomorrow 00:00)
• HOT is downloaded once (all channels, whole day) then filtered.
• Skips channels whose xmltv_id is empty; first provider with data wins.
• Programme timestamps are written in UTC (+0000).
Output: file2.xml
"""
...
# ───────── FreeTV / Cellcom / Partner / Yes (unchanged) ─────────
def fetch_freetv(sess, sid, since, till):
    ...
    for a in (1, 2):
        r = sess.get(FREETV_API, params=p, timeout=30); dbg("freetv.tv", r.url)
...
def _cell_req(sess, ks, chan, sts, ets, quoted):
    ...
    r = sess.post(CELL_LIST, json=payload, headers=CELL_HEADERS, timeout=30); dbg("cellcom.co.il", r.url); r.raise_for_status()
    return r.json()
...
def fetch_cellcom(sess, site_id, since, till):
    ...
    r = sess.post(CELL_LOGIN, json={"apiVersion": "5.4.0.28193", "partnerId": "3197", "udid": "f442..."},
                  headers=CELL_HEADERS, timeout=30); dbg("cellcom.co.il", r.url); r.raise_for_status()
...
def fetch_partner(sess, site_id, since, _):
    ...
    r = sess.post(PARTNER_EPG, json=body, headers=PARTNER_HEADERS, timeout=30); dbg("partner.co.il", r.url); r.raise_for_status()
...
def fetch_yes(sess, site_id, since, _):
    ...
    r = sess.get(url, headers=YES_HEADERS, timeout=30); dbg("yes.co.il", r.url); r.raise_for_status()
    return r.json().get("items", [])

# ───────── HOT (single download, cached) ─────────
_HOT_CACHE = None
def _collect_hot_day(sess, day_start):
    ...
    payload = {"ChannelId": "0",
               ...
               "Hour": 0}
    r = sess.post(HOT_API, json=payload, headers=HOT_HEADERS, timeout=60); dbg("hot.net.il", r.url)
    try:
    ...
    by = {}
    for it in rows:
        if not it.get("programStartTime", "").startswith(today):
            continue
...
def fetch_hot(sess, site_id, day_start, _):
    global _HOT_CACHE
    if _HOT_CACHE is None:
        _HOT_CACHE = _collect_hot_day(sess, day_start)
...
def build_epg():
    ...
    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        ...
        groups.setdefault(xmltv, []).append((site, raw, name, logical))

    for xmltv, variants in groups.items():
        items = None; site_used = ""; name = variants[0][2]
        for site, raw, _, logical in variants:
            ...
            if items:
                site_used = site; break
        if not items:
            dbg("skip", f"{xmltv}: no data", flush=True); continue

        CH = ET.SubElement(root, "channel", id=xmltv)
        ET.SubElement(CH, "display-name", lang="he").text = name
        ...
        for it in items:
            try:
                if site_used=="freetv.tv":
                    ...
                    # ← ONLY CHANGE: mark IL zone *before* converting to UTC -------------
                    s = datetime.strptime(it["programStartTime"], HOT_DT).replace(tzinfo=IL_TZ).astimezone(dt.timezone.utc)
                    e = datetime.strptime(it["programEndTime"],   HOT_DT).replace(tzinfo=IL_TZ).astimezone(dt.timezone.utc)
                ...
                dbg(site_used, "programme error", xmltv, e, flush=True)
