...
Channels are written first; all programme elements follow.
Output: file2.xml
"""
...
PARTNER_HEADERS = {"Content-Type": "application/json;charset=UTF-8","Accept": "application/json, text/plain, */*",
                   "brand":"orange","category":"TV","platform":"WEB","subCategory":"EPG",
                   "lang":"he-il","Accept-Encoding":"gzip,deflate,br","User-Agent": UA}
...
_DBG = os.getenv("DEBUG","1") not in ("0","false","no")
...
def fetch_cellcom(sess,site_id,since,till):
    ...
    obj = d.get("objects") or d.get("result",{}).get("objects",[])
    if obj: return obj
...
def _collect_hot_day(sess,start):
    ...
    for it in rows:
        if not str(it.get("programStartTime","")).startswith(today): continue
...
def build_epg():
    ...
    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        ...
        grouped.setdefault(xmltv,[]).append((ch.attrib.get("site","").lower(),ch.attrib["site_id"],(ch.text or xmltv).strip()))

    for xmltv, variants in grouped.items():
        best_items,best_site,best_name=[],None,variants[0][2]
        ...
        for it in best_items:
            try:
                ...
                dbg(best_site,"programme error",xmltv,e,flush=True)
