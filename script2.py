...
Times are written to XMLTV in **UTC (+0000)**.
Output: file2.xml
"""
...
def dbg(site: str, *msg, flush: bool = False):
    if _DBG:
...
def build_epg():
    ...
    # group all rows by xmltv_id (skip empty)
    ...
    for xmltv, variants in entries.items():
        ...
        ET.SubElement(root, "channel", id=xmltv)
        ET.SubElement(root[-1], "display-name", lang="he").text = name
