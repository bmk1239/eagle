...
def fetch_cellcom(sess, site_id, since, till):
    ...
    if d.get("result", {}).get("error", {}).get("code") == "4004":
        d2 = _cell_req(sess, ks, chan, sts, ets, True)
        return d2.get("objects") or d2.get("result", {}).get("objects", [])
    return []

# ───────── Partner ─────────
def fetch_partner(sess, site_id, since, _):
    param = f"{site_id}|{since:%Y-%m-%d}|UTC"
...
# ───────── HOT (24 hourly buckets) ─────────
HOT_DT = "%Y/%m/%d %H:%M:%S"
def fetch_hot(sess, site_id, start, _):
    chan = site_id.zfill(3)
    seen: set[str] = set()
    out:  list     = []

    for hr in range(24):
        payload = {"ChannelId": chan,
                   "ProgramsStartDateTime": (start + dt.timedelta(hours=hr)).strftime("%Y-%m-%dT%H:00:00"),
                   "ProgramsEndDateTime":   (start + dt.timedelta(hours=hr+1)).strftime("%Y-%m-%dT%H:00:00"),
                   "Hour": hr}
        r = sess.post(HOT_API, json=payload, headers=HOT_HEADERS, timeout=30)
        if _DBG: print(r.url, flush=True)
        try:
    ...
    dbg("hot.net.il", f"channel {chan} items: {len(out)}", flush=True)
    return out
...
def build_epg():
    ...
    for xmltv, variants in entries.items():
        chosen_items = chosen_site = chosen_name = None
        for site, raw, name in variants:
            ...
            if items:
                chosen_items, chosen_site, chosen_name = items, site, name
                break
        if not chosen_items:
