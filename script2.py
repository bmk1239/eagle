#!/usr/bin/env python3
"""
XMLTV guide builder for FreeTV + Cellcom, routed through an Israeli proxy.
• Every HTTP request URL is printed.                • DEBUG=1 adds [DBG] lines
• Cellcom auto-retries with quoted timestamps if numeric fails.
• Duplicate logical channel-IDs are skipped only after at least one variant
  has delivered data.                                • Output file: file2.xml
"""
...
class InsecureTunnel(HTTPAdapter):
    """TLS-disabled adapter for CONNECTing through an upstream proxy."""
    def _ctx(self):
...
def to_dt(val):
    """Convert ISO string or unix-seconds int/float to aware datetime in IL_TZ."""
...
def build_epg():
    ...
    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        ...
        if key not in id_state:                 # first encounter → write channel
            CE = ET.SubElement(root, "channel", id=xmltv_id)
            ET.SubElement(CE, "display-name", lang="he").text = name

        if id_state.get(key):                   # already have data → skip
    ...
    # write <programme> after channels
