...
# ───────── Cellcom (no date filter in KSQL) ─────────
...
def fetch_cellcom(sess,site_id,since,till):
    ...
    # keep only items inside our window
    out=[]
    for it in objs:
        s=to_dt(it["startDate"]); e=to_dt(it["endDate"])
        if e>since and s<till:
            out.append(it)
    return out
...
def build_epg():
    ...
    for xmltv,variants in entries.items():
        data=None; site_used=name_used=None
        for site,raw,name in variants:
            ...
            if data:
                site_used,name_used=data,site,name
                break
        if not data: continue

        ET.SubElement(root,"channel",id=xmltv).append(
            ET.Element("display-name",lang="he",text=name_used)
        )
        ...
        for it in data:
            try:
                ...
                progs.append((s,e,title,desc,site,xmltv))
            except Exception as e:
