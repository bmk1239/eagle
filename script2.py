...
• One-day guide: IL-today 00:00 → tomorrow 00:00
• Single HOT download (all channels, whole day) – filter to ‘today’
• Skips channel lines whose xmltv_id is empty
• First site that returns data for a logical channel wins; later duplicates ignored
...
SLASH_FMT  = "%d/%m/%Y %H:%M"
HOT_FMT    = "%Y/%m/%d %H:%M:%S"   # no TZ info – *local IL time*

def to_dt(v: str | int | float) -> dt.datetime:
    """Parse FreeTV/Cellcom/Partner/Yes datetimes (various formats) → aware IL dt."""
...
def fetch_cellcom(sess, site_id, since, till):
    ...
    ks = sess.post(CELL_LOGIN, json=login, headers=CELL_HEADERS, timeout=30).json().get("ks")
    d  = _cell_req(sess, ks, chan, sts, ets, False);  objs = d.get("objects") or d.get("result", {}).get("objects", [])
...
def _collect_hot_day(sess, start):
    ...
    ch_map: dict[str, list] = {}
    for it in rows:
        if str(it.get("programStartTime", "")).startswith(today):   # only today
            cid = str(it.get("channelID", "")).zfill(3)
            ch_map.setdefault(cid, []).append(it)
    dbg("hot.net.il", f"rows kept: {sum(len(v) for v in ch_map.values())}", flush=True)
    return ch_map
...
def build_epg():
    ...
    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        ...
        grouped.setdefault(xmltv_id, []).append((site, site_id, name))

    for xmltv_id, variants in grouped.items():
        items = []; origin_site = ""; disp_name = variants[0][2]
        for site, sid, _ in variants:
            try:
                ...
                dbg(site, "fetch error", xmltv_id, e, flush=True);  items = []
            if items:
                origin_site = site;  break
        if not items:
            continue
        ...
        for it in items:
            try:
                ...
                start_utc = s.astimezone(timezone.utc).strftime("%Y%m%d%H%M%S +0000")
                end_utc   = e.astimezone(timezone.utc).strftime("%Y%m%d%H%M%S +0000")

                p = ET.SubElement(root, "programme", start=start_utc, stop=end_utc, channel=xmltv_id)
                ET.SubElement(p, "title", lang="he").text = escape(title)
                if desc:
                    ET.SubElement(p, "desc", lang="he").text = escape(desc)
            except Exception as e:
                dbg(origin_site, "programme error", xmltv_id, e, flush=True)
