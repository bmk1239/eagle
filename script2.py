...
Creates a 1-day guide, from 00:00 IL today to 00:00 tomorrow.
Output: file2.xml
"""
...
# ── tiny debug helper (now tolerant to flush=True) ──────────────
...
# ── HOT (single request for the whole day, cached) ──────────────
_HOT_CACHE: dict[str, list] | None = None          # filled only once per run
_HOT_DT = "%Y/%m/%d %H:%M:%S"                      # 2025/07/02 03:00:00

def _collect_hot_day(sess, day_start: dt.datetime) -> dict[str, list]:
    """Call HOT once (Hour=0) and split the huge list by channelID."""
    ...
    rows = jd.get("data", {}).get("programsDetails", [])
    dbg("hot.net.il", f"whole-day rows: {len(rows)}", flush=True)
    by_chan: dict[str, list] = {}
    for item in rows:
        cid = str(item.get("channelID", "")).zfill(3)
        by_chan.setdefault(cid, []).append(item)
    return by_chan


def fetch_hot(sess, site_id: str, since: dt.datetime, _till: dt.datetime):
    """Return EPG rows for one HOT channel, using cached day content."""
    global _HOT_CACHE
    if _HOT_CACHE is None:
        _HOT_CACHE = _collect_hot_day(sess, since)
    chan_items = _HOT_CACHE.get(site_id.zfill(3), [])
    dbg("hot.net.il", f"channel {site_id} items: {len(chan_items)}", flush=True)
    return chan_items
    ...
    for xid, it, site in programmes:
        try:
            if site == "freetv.tv":
                ...
                s = dt.datetime.strptime(it["programStartTime"], _HOT_DT).replace(tzinfo=IL_TZ)
                e = dt.datetime.strptime(it["programEndTime"],   _HOT_DT).replace(tzinfo=IL_TZ)
