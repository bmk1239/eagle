...
def fetch_hot(sess, site_id, start, till):
    ...
    while cur < till:
        day_key = cur.date()
        if day_key not in _HOT_CACHE:
            _HOT_CACHE[day_key] = _collect_hot_day(sess, cur)
        items.extend(_HOT_CACHE[day_key].get(site_id.zfill(3), []))
        cur += dt.timedelta(days=1)
...
def build_epg():
    ...
    all_programs: list[tuple[dt.datetime, dt.datetime, str, str, str, str]] = []
        ...
        for it in items:
            try:
                if site == "freetv.tv":
                    ...
                    title, desc = it["name"], it.get("shortSynopsis")
