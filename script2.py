#!/usr/bin/env python3
"""Combined FreeTV + Cellcom EPG builder (proxy). Prints each request URL."""
...
def fetch_cellcom(sess,site_id,since,till):
    ...
    login_json={"apiVersion":"5.4.0.28193","partnerId":"3197","udid":"f4423331-81a2-4a08-8c62-95515d080d79"}
    r=sess.post(CELL_LOGIN,json=login_json,headers=CELL_HEADERS,timeout=30); print(r.url); r.raise_for_status()
    ks=r.json().get("ks") or r.json().get("result",{}).get("ks")
    if not ks: raise RuntimeError("KS missing")

    since_ts=int(since.timestamp()); till_ts=int(till.timestamp())
    payload={"apiVersion":"5.4.0.28193","clientTag":"2500009-Android",
             "filter":{"kSql":f\"(and epg_channel_id='{chan}' start_date>{since_ts} end_date<{till_ts} asset_type='epg')\",
    ...
    if not objs: dbg("Cellcom raw response",json.dumps(data)[:500]+"…")
    return objs
