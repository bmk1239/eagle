...
def day_window(now: dt.datetime):
    """Return (since, till) covering current Sun-to-Sun week in IL timezone."""
    weekday = now.weekday()          # Monday=0 … Sunday=6
    days_from_sun = (weekday + 1) % 7   # 0 if Sunday, 1 if Monday, …
...
def build_epg():
    ...
    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        ...
        if not xmltv:
            dbg("skip","empty xmltv_id, ignored",flush=True); continue
    ...
    for xmltv, variants in entries.items():
        ...
        if not ok_items:
            dbg("skip",f"{xmltv} no data from any variant",flush=True); continue
