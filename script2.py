...
Creates a 1-week guide (Sunday 00:00 → next Saturday 23:59).
Output: file2.xml
"""
...
def day_window(now):
    # Calculate the Sunday (start of week) and next Saturday (end of week)
    weekday = now.weekday()  # Monday=0, Sunday=6; so Sunday=6 or 0?
    # Adjust: Python Monday=0 ... Sunday=6
    # We want Sunday as start: if today is Sunday (6), start today; else previous Sunday
    days_to_sunday = (now.weekday() + 1) % 7  # converts Mon=0 to Sun=6 → Sunday=0
    sunday = dt.datetime.combine((now - dt.timedelta(days=days_to_sunday)).date(), dt.time.min, tzinfo=IL_TZ)
