...
Creates a 1-day guide (IL-today 00 → IL-tomorrow 00).
Output: file2.xml
"""
...
import json, ssl, cloudscraper, urllib3
...
class InsecureTunnel(HTTPAdapter):
    def _ctx(self):
        c = create_urllib3_context(); c.check_hostname = False; c.verify_mode = ssl.CERT_NONE
        return c
    def init_poolmanager(self,*a,**kw): kw["ssl_context"]=self._ctx(); return super().init_poolmanager(*a,**kw)
...
CELL_HEADERS   = {"User-Agent": UA, "Content-Type": "application/json", "Accept-Encoding": "gzip, deflate, br"}
PARTNER_HEADERS= {"User-Agent": UA, "Content-Type": "application/json;charset=UTF-8",
                  "Accept":"application/json, text/plain, */*", "brand":"orange","category":"TV",
                  "platform":"WEB","subCategory":"EPG","lang":"he-il","Accept-Encoding":"gzip,deflate,br"}
YES_HEADERS    = {"User-Agent": UA, "Accept":"application/json, text/plain, */*",
                  "Accept-Language":"he-IL","Referer":"https://www.yes.co.il/","Origin":"https://www.yes.co.il"}
HOT_HEADERS    = {"User-Agent": UA, "Content-Type":"application/json",
...
def dbg(site,*msg,flush=False):
    if _DBG: print(f"[DBG {site}]",*msg,flush=flush)

# ─────────── small utils ───────────
_Z_RE = re.compile(r"Z$")
_SLASH_FMT = "%d/%m/%Y %H:%M"
def to_dt(val):
    if isinstance(val,(int,float)): return dt.datetime.fromtimestamp(int(val),tz=IL_TZ)
    if isinstance(val,str):
        if "/" in val: return datetime.strptime(val,_SLASH_FMT).replace(tzinfo=IL_TZ)
        return dt.datetime.fromisoformat(_Z_RE.sub("+00:00",val)).astimezone(IL_TZ)
...
# ─────────── Cellcom ───────────
def _cell_req(sess,ks,chan,sts,ets,quote):
...
def fetch_cellcom(sess,site_id,since,till):
    ...
    login={"apiVersion":"5.4.0.28193","partnerId":"3197","udid":"f4423331-81a2-4a08-8c62-95515d080d79"}
...
def _collect_hot_day(sess,start):
    ...
    except Exception as e: dbg("hot.net.il","json error",e,flush=True); return {}
...
def build_epg():
    ...
    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        ...
        if not xml_id: dbg("skip","empty xmltv_id ignored",flush=True); continue
        ...
        grouped.setdefault(xml_id,[]).append((site,raw,name,logical))
    ...
    for xml_id, variants in grouped.items():
        ...
        if not items: dbg("skip",f"{xml_id}: no data",flush=True); continue

        CE=ET.SubElement(root,"channel",id=xml_id); ET.SubElement(CE,"display-name",lang="he").text=disp_name
        ...
        for it in items:
            try:
                ...
                # ---------- always emit UTC ----------
                s_utc,e_utc=s.astimezone(dt.timezone.utc),e.astimezone(dt.timezone.utc)
