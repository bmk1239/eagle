#!/usr/bin/env python3
"""
EPG builder for FreeTV, Cellcom, Partner and Yes via an Israeli proxy.
• Prints every request URL; DEBUG=1 shows [DBG <site>] … messages.
• Duplicate logical channel-IDs are skipped only after one returns data.
• Output file: file2.xml
"""

from __future__ import annotations
import datetime as dt, os, json, warnings, re, xml.etree.ElementTree as ET
...
def fetch_freetv(sess,sid,since,till):
    ...
    for a in (1,2):
        r=sess.get(FREETV_API,params=p,timeout=30); print(r.url)
...
def _cell_req(sess,ks,chan,sts,ets,quoted):
    ...
    r=sess.post(CELL_LIST,json=payload,headers=CELL_HEADERS,timeout=30); print(r.url); r.raise_for_status(); return r.json()
...
def fetch_cellcom(sess,site_id,since,till):
    ...
    r=sess.post(CELL_LOGIN,json=login,headers=CELL_HEADERS,timeout=30); print(r.url); r.raise_for_status()
...
def fetch_partner(sess,site_id,since,till):
    chan=site_id.strip(); param=f"{chan}|{since.strftime('%Y-%m-%d')}|UTC"
    body={"_keys":["param"],"_values":[param],"param":param}
    r=sess.post(PARTNER_EPG,json=body,headers=PARTNER_HEADERS,timeout=30); print(r.url); r.raise_for_status()
...
def fetch_yes(sess,site_id,since,till):
    chan=site_id.strip(); url=f"{YES_CH_BASE}/{chan}?date={since.strftime('%Y-%m-%d')}&ignorePastItems=true"
    r=sess.get(url,headers=YES_HEADERS,timeout=30); print(r.url); r.raise_for_status()
    return r.json().get("items",[])

# ───────── main ─────────
def build_epg():
    since,till=day_window(dt.datetime.now(IL_TZ)); sess=new_session()
    root=ET.Element("tv",{"source-info-name":"FreeTV+Cellcom+Partner+Yes",
                          "generator-info-name":"proxyEPG"})
    programmes=[]; id_state:dict[tuple[str,str],bool]={}

    for ch in ET.parse(CHANNELS_FILE).findall("channel"):
        site  = ch.attrib.get("site","").lower()
        raw   = ch.attrib["site_id"]; logical=raw.split("##")[0] if site=="cellcom.co.il" else raw
        xmltv = ch.attrib["xmltv_id"]; name=(ch.text or xmltv).strip(); key=(site,logical)

        if key not in id_state: CE=ET.SubElement(root,"channel",id=xmltv); ET.SubElement(CE,"display-name",lang="he").text=name
        if id_state.get(key):   dbg(site,"skip duplicate with data",key); continue
        ...
        if items: id_state[key]=True; programmes.extend((xmltv,it,site) for it in items)
    ...
    for xid,it,site in programmes:
        try:
            ...
            pr=ET.SubElement(root,"programme",start=s.strftime("%Y%m%d%H%M%S %z"),
                             stop=e.strftime("%Y%m%d%H%M%S %z"),channel=xid)
